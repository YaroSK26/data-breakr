...
function FinancnyBenchmark() {
  ...
  const [okresParam, setOkresParam] = useState(
    () => searchParams.get("okres") ?? "",
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(
      `/api/benchmark?nace=${encodeURIComponent(naceParam)}&okres=${encodeURIComponent(okresParam)}`,
    )
      .then((r) => r.json())
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 14, color: "#78350f" }}>
          <strong>Tieto dáta pokrývajú najmä s.r.o. a a.s.</strong> Živnostníci
          sú zastúpení len čiastočne a nereprezentatívne - iba tí, ktorí
          dobrovoľne vedú jednoduché alebo podvojné účtovníctvo. Väčšina
          živnostníkov (na paušálne výdavky alebo daňovú evidenciu) v týchto
          dátach absentuje úplne. Berte preto tieto čísla ako orientačné, nie
          ako presný obraz celého odvetvia.
        </div>
      </div>
      ...
      >
        ...
        {loading && (
          <span style={{ color: "#94a3b8", fontSize: 13 }}>Načítavam…</span>
        )}
      </section>
      ...
      {!naceParam || !okresParam ? (
        ...
        >
          Pre túto kombináciu odvetvia a okresu nemáme žiadne dáta z účtovných
          závierok.
        </div>
      ) : rows ? (
        <div
        ...
        >
                ...
                return (
                  <tr key={r.rok} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    {anonymized ? (
                      ...
                      >
                        Nedostatok dát na zobrazenie (medián z {r.firmCount}{" "}
                        firiem - pod hranicou anonymity)
                      </td>
                    ) : (
                      <>
                        <td
          ...
          <p style={{ fontSize: 12, color: "#94a3b8", marginTop: 8 }}>
            Medián tržieb na zamestnanca nie je zobrazený - dáta o počte
            zamestnancov nie sú v tomto zdroji k dispozícii (RÚZ obsahuje len
            veľkostnú kategóriu, nie presný počet).
          </p>
        </div>
      ) : null}

      <div style={{ marginTop: 24 }}>
        <DataSourceBanner sources={ruzSource ? [ruzSource] : []} />
      </div>
    </main>
  );
}
