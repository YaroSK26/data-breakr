...
export function FirmListPanel({ okresKod, okresNazov, naceFilter, categories, onClose }: FirmListPanelProps) {
  ...
  useEffect(() => {
    setLoading(true)
    const params = new URLSearchParams({ okres: okresKod, offset: String(offset) })
  ...
  const categoryName = (kod4: string | null) => categories.find((c) => c.kod4 === kod4)?.nazov ?? kod4 ?? '—'
    ...
    >
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <button
            onClick={onClose}
            style={{ border: 'none', background: 'none', fontSize: 20, cursor: 'pointer', color: '#64748b', lineHeight: 1 }}
            aria-label="Zavrieť"
          >
            ×
          </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
        {loading ? (
          ...
          <p style={{ padding: '16px 20px', color: '#94a3b8' }}>Žiadne firmy pre tento výber.</p>
        ) : (
          firms.map((f) => (
            <div key={f.id} style={{ padding: '10px 20px', borderBottom: '1px solid #f1f5f9' }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>{f.nazov ?? '(bez názvu)'}</div>
              <div style={{ fontSize: 12, color: '#64748b' }}>
                {[f.ulica, f.mesto, f.psc].filter(Boolean).join(', ') || '—'}
              </div>
              {!localNace && (
                <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{categoryName(f.naceKod4)}</div>
              )}
            </div>
          ))
        )}
      ...
      {total > 100 && (
        <div style={{ padding: '10px 20px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13 }}>
          <button
            onClick={() => setOffset((o) => Math.max(0, o - 100))}
            disabled={offset === 0}
            style={{ padding: '4px 10px', cursor: offset === 0 ? 'default' : 'pointer', opacity: offset === 0 ? 0.4 : 1 }}
          >
            ← predošlé
          </button>
          <span style={{ color: '#64748b' }}>
            {offset + 1}–{Math.min(offset + 100, total)} z {total.toLocaleString('sk-SK')}
          </span>
          <button
            onClick={() => setOffset((o) => o + 100)}
            disabled={offset + 100 >= total}
            style={{ padding: '4px 10px', cursor: offset + 100 >= total ? 'default' : 'pointer', opacity: offset + 100 >= total ? 0.4 : 1 }}
          >
            ďalšie →
          </button>
        </div>
