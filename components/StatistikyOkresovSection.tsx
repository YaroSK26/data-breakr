...
export function StatistikyOkresovSection() {
  ...
  return (
    <section id="statistiky-okresov" style={{ marginTop: 40, scrollMarginTop: 20 }}>
      <header style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>
          Štatistiky okresov
        </h2>
        <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: 14 }}>
          Mzdy, zamestnanosť a počty podnikov podľa okresu a odvetvia.
          Oficiálne údaje Štatistického úradu SR.
        </p>
      </header>

      {/* Pravidlo o transparentnosti z CLAUDE.md - toto obmedzenie mení
          význam čísel, takže patrí nad ne, nie pod ne. Rovnaká neutrálna
          paleta ako DataSourceBanner - nejde o varovanie, len o vysvetlenie
          zdroja, tak nemá dôvod vyzerať ako výstraha. */}
      <div
        style={{
          ...CARD,
          borderColor: "#cbd5e1",
      ...
      >
        ...
        Rev. 2 v sekcii J. Preto sa tieto čísla nedajú priamo sčítať s
        počtami firiem z mapy hustoty vyššie. Mzdy a zamestnanci sa navyše
        zisťujú len v podnikoch s 20 a viac zamestnancami, čiže živnostníkov
        a malé firmy nepokrývajú.
      </div>
      ...
      >
        <div style={{ flex: "1 1 240px" }}>
          ...
          <select
            style={SELECT}
            value={ukazovatel}
            onChange={(e) => setUkazovatel(e.target.value)}
          >
            {UKAZOVATELE.map((u) => (
      ...
      <section style={{ ...CARD, padding: 8 }}>
        ...
        >
          Dáta dostupné pre {data?.okresy.length ?? 0} z 79 okresov. Farba
          obce zodpovedá hodnote jej okresu - ŠÚ SR tieto ukazovatele
          zverejňuje po okresy, nie po obce. Zvyšné okresy sú biele: pri
          malom počte podnikov v danom odvetví ŠÚ SR hodnotu netají len na
          mape, ale ju vôbec nezverejní (napr. „B - Ťažba a dobývanie" má v
          mnohých okresoch nula alebo len pár podnikov).
        </p>
      </section>
                ...
                {tabulka.data.length === 0 && (
                  <tr>
                    <td style={{ padding: "7px 4px", color: "#94a3b8" }}>
                      {loading ? "Načítavam…" : "Za tento výber nie sú zverejnené údaje."}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <DataSourceBanner
        ...
        note={
          <>
            Údaje sú ročné a ŠÚ SR ich spätne reviduje - pri každom
            sťahovaní sa celý ukazovateľ prepisuje nanovo, nie dopĺňa.
          </>
        }
      />
    </section>
